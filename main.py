...
async def on_ready():
    ...
    try:
        ...
        logging.error(f"Error loading Radio cog: {error}")

    try:
        await bot.load_extension('src.cogs.voice_manager')
        logging.info("Voice manager cog loaded successfully.")
    except Exception as error:
        logging.error(f"Error loading Voice Manager cog: {error}")
...
if __name__ == "__main__":
    ...
    if not token:
        raise Exception(
            "No Discord API Token available. Please set the DISCORD_API_TOKEN environment variable.")
    else:
        bot.run(token)
