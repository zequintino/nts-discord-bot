"""NTS Radio API interaction module"""
import aiohttp
import html
...
def extract_show_details(channel_data):
    ...
    show_name = channel_data.get('now', {}).get(
        'broadcast_title', 'Unknown Show')
    location_short = channel_data.get('now', {}).get(
        'embeds', {}).get('details', {}).get('location_short')
    location_long = channel_data.get('now', {}).get(
        'embeds', {}).get('details', {}).get('location_long')
...
class NTSRadioInfo:
    ...
    async def get_both_channels_info():
        ...
        channel1_details = extract_show_details(
            channel1_data) if channel1_data else None
        channel2_details = extract_show_details(
            channel2_data) if channel2_data else None
